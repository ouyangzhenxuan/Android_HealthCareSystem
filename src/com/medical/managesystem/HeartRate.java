...
public class HeartRate extends Activity {
	...
	//临时变量使用
	public int data_i2;
	private MyApplicationDataSave2 mAppHR;
	private DrawThread drawThread_hr;
    @Override  
    protected void onCreate(Bundle savedInstanceState) {  
        super.onCreate(savedInstanceState);  
        setContentView(R.layout.heartrate);  
        mAppHR = new MyApplicationDataSave2();
        DrawChart();
        drawThread_hr = new DrawThread();
		if(DeviceRegister.connect_state[2]==true) {//判断改传感器连接状态
			drawThread_hr.start();
		}else {
			alertNodata().show();
		}
    ...
    private  AlertDialog alertNodata() {
		AlertDialog noresponse = new AlertDialog.Builder(HeartRate.this)
				.setTitle("提示")
				.setMessage("现在暂无数据，需要继续等待还是返回？")
				.setNegativeButton("返回", new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						drawThread_hr.interrupt();
						finish();
					}
				})
				.setPositiveButton("等待", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						new Handler().postDelayed(new Runnable() {
				            @Override
				            public void run() {
				            	if(DeviceRegister.connect_state[2]==true) {
									drawThread_hr.start();
								}else {
									drawThread_hr.interrupt();
    		...
    		public void run() {
    			for(int i=0;;i++) {
    				try {
    					...
    					Log.d("mAppBP.getData(data_i)", ""+mAppHR.getData(data_i2));
    					if(mAppHR.getData(data_i2)!=null) {
    					  Message msg = new Message();
    					  msg.obj = mAppHR.getData(data_i2);
    	...
    	private void DrawChart() {
    		mChart = (LineChart) findViewById(R.id.chart_hr);  
